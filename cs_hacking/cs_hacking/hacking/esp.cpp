...
CGameTrace TraceShape(Vector3& vecStart, Vector3& vecEnd, uintptr_t pSkipEntity, const uint64_t nMask)
{
	...
	TTaceRay pfnTraceShape = (TTaceRay)(client + 0x6E4D10);
	Filter pfnCreateFilter = (Filter)(client + 0x1FA1A0);
	void* pTraceManager = *(void**)(client + 0x1BB9948);
...
void draw_esp() {
	...
	for (int i = 0; i < 64; i++) {
		...
		//是否头盔
		auto player_bHasHelmet = *reinterpret_cast<bool*>(player_pawn + cs2_dumper::schemas::client_dll::CCSPlayer_ItemServices::m_bHasHelmet);
		//玩家三维坐标
		...
		if (cs::visuals::box)
		{
			if (cs::visuals::SelectedBoxType == 0)
			{
				...
				if (bVisible)
				{
					...
					DrawList->AddRect(ImVec2(x, y), ImVec2(x + width, y + height), ImColor(cs::visuals::FillConvex), RandomPara<float>(0.0f, 3.0f), 0, 1.3);
				}
			}
			else
			{
				...
				if (bVisible)
				{
					...
					DrawList->AddLine(ImVec2(x, y), ImVec2(x +  width * 0.25f, y), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x, y), ImVec2(x, y + height * 0.25f), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x + width, y), ImVec2(x + width -  width * 0.25f, y), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x + width, y), ImVec2(x + width, y + height * 0.25f), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x, y + height), ImVec2(x +  width * 0.25f, y + height), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x, y + height), ImVec2(x, y + height - height * 0.25f), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x + width, y + height), ImVec2(x + width -  width * 0.25f, y + height), ImColor(cs::visuals::FillConvex), 1.3f);
					DrawList->AddLine(ImVec2(x + width, y + height), ImVec2(x + width, y + height - height * 0.25f), ImColor(cs::visuals::FillConvex), 1.3f);
				}
			}
		}
		//骨骼
		...
		if (cs::visuals::AmmoBar) {
			float AmmoBarProportion = static_cast<float>(player_iAmmo1) / player_pReserveAmmo1;
		...
		if (cs::visuals::HeadCircle) {
			DrawList->AddCircle(ImVec2(head_pos.x, head_pos.y), Radius, ImColor(cs::visuals::HeadCircleColor));
		}
		//距离
		...
		if (cs::visuals::weapon) {
			...
			DrawList->AddText(ImVec2(head_pos_2d.x - 12, y - 10 ), ImColor(cs::visuals::weaponColor), weaponName.c_str());
		}
		/*if (cs::visuals::name) {
			DrawList->AddText(ImVec2(x + (width / 2.f) , y + 6), ImColor(cs::visuals::nameColor), reinterpret_cast<const char*>(player_name));
		}*/
