...
ImVec2 GetScreenCenter() {
    ...
    return ImVec2(size.x / 2.0f, size.y / 2.0f);
}
...
void startAimbot(Vector3 headScreen, float smoothness)
{
    ...
    moveMouseByOffset(-(center.x - headScreen.x) / smoothness, -(center.y - headScreen.y) / smoothness);
}

void drawFovCircle(float radius)
{
    ImVec2 center = GetScreenCenter();
    ImGui::GetBackgroundDrawList()->AddCircle(center, radius, ImColor(255, 255, 255, 255), 25, 2.f);
}
...
{
    uintptr_t clipping_weapon = HV::Read<uintptr_t>(playerPawn + cs2_dumper::schemas::client_dll::C_CSPlayerPawn::m_pClippingWeapon);
